...
import { Compass, ArrowRight, Send, Loader2 } from "lucide-react";
...
export function GuideMe() {
  ...
  // === INPUT VIEW ===
  return (
    <div className="bg-card border border-border rounded-xl p-5 sm:p-6 shadow-lg space-y-4">
      <div>
        <h2 className="text-base font-display font-semibold text-foreground">
          Tell us why you are here today
        </h2>
        <p className="text-xs text-muted-foreground mt-1">
          Describe what you are dealing with and we will point you to the right part of the site.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {QUICK_OPTIONS.map((opt) => (
          <button
            ...
            className="text-xs px-3 py-1.5 rounded-full border border-border bg-background text-foreground hover:bg-primary/5 hover:border-primary/30 transition-all text-left"
          >
            {opt}
          </button>
        ))}
